...
bool CAlg_Excel::OnOpenMotorPosition(CString strFilename)
{
	...
	for(i=0; i<m_nPartCnt; i++)
	{
		for(j=0; j<m_nPartAxisCnt[i]; j++)
		{
			strData									= m_clsExcel->GetCellValueCString((m_nPartAxis[i][j] * 3) + 2, 16);
			m_nAxisPosNum[i][j]						= _wtoi(strData);

			strData									= m_clsExcel->GetCellValueCString((m_nPartAxis[i][j] * 3) + 2, 3);
			nAxis									= _wtoi(strData);

			strData									= m_clsExcel->GetCellValueCString((m_nPartAxis[i][j] * 3) + 2, 8);
//			m_nAxisMinusEl[i][j]					= _wtoi(strData);
			m_nAxisLimitInfo[i][j][0]				= _wtoi(strData);

			strData									= m_clsExcel->GetCellValueCString((m_nPartAxis[i][j] * 3) + 2, 9);
//			m_nAxisPlusEl[i][j]						= _wtoi(strData);
			m_nAxisLimitInfo[i][j][1]				= _wtoi(strData);

			strData									= m_clsExcel->GetCellValueCString((m_nPartAxis[i][j] * 3) + 2, 10);
//			m_nAxisPlusEl[i][j]						= _wtoi(strData);
			m_nAxisLimitInfo[i][j][2]				= _wtoi(strData);

			strData									= m_clsExcel->GetCellValueCString((m_nPartAxis[i][j] * 3) + 2, 7);
//			m_nAxisHome[i][j]						= _wtoi(strData);
			m_nAxisLimitCnt[i][j]					= _wtoi(strData);

			strData									= m_clsExcel->GetCellValueCString((m_nPartAxis[i][j] * 3) + 2, 11);
			m_nAxisDirection[i][j]					= _wtoi(strData);

			strData									= m_clsExcel->GetCellValueCString((m_nPartAxis[i][j] * 3) + 2, 12);
			m_nAxisAccount[i][j]					= _wtoi(strData);

			for(k=0; k<m_nAxisPosNum[i][j]; k++)
			{
				m_strAxisPosName[i][j][k]			= m_clsExcel->GetCellValueCString((m_nPartAxis[i][j] * 3) + 2, k+17);
				strData								= m_clsExcel->GetCellValueCString(((m_nPartAxis[i][j] * 3) + 2)+1, k+17);
				m_nAxisPosData[nAxis][k]			= _wtoi(strData);
				m_strAxisPosInfo[i][j][k]			= m_clsExcel->GetCellValueCString(((m_nPartAxis[i][j] * 3) + 2)+2, k+17);
			}
		}
	}

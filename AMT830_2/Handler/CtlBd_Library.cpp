...
//		nMotor[0] = M_BARCODE_Y;
	...
	//	nMotor[0] = M_BARCODE_Z;
		...
		// loader tray shifter
	//	nMotor[0] = M_LD_TRAY_SHIFTER;	
		// 1mm 이동하는데 필요한 pulse
		COMI.Set_MotUnitDist(nMotor[0], 518.588931547394);
		// 10mm 이동하는데 필요한 pulse (속도는 1Cm/초로 세팅하게 된다.)
		COMI.Set_MotUnitSpeed(nMotor[0], 518.588931547394);
		// motor Home
		COMI.mn_homecheck_method[nMotor[0]]= 6;

		// unload tray shifter
		//nMotor[0] = M_UNLD_TRAY_SHIFTER;	
		// 1mm 이동하는데 필요한 pulse
		COMI.Set_MotUnitDist(nMotor[0], 518.588931547394);
		// 10mm 이동하는데 필요한 pulse (속도는 1Cm/초로 세팅하게 된다.)
		COMI.Set_MotUnitSpeed(nMotor[0], 518.588931547394);
...
void CCtlBd_Library::Motor_Error_Occurrence(int n_Mode, long l_Axis, long l_ErrorIDNum, CString strErrorMsg)//20150420 james 모터 리턴 에러 메세지 출력 
{
	...
	case -1: //사용하지 않는 루틴
		...
		break;
	case 0: //0번모터 
		strMsg_1.Format(_T("M_BARCODE_X[%02ld]"), l_Axis);
		break;
	case 1:  //1번모터 
		strMsg_1.Format(_T("M_BARCODE_Y[%02ld]"), l_Axis);
		break;
	case 2:  //2번모터 
		strMsg_1.Format(_T("M_BARCODE_Z[%02ld]"), l_Axis);
		break;
	case 3:   
		strMsg_1.Format(_T("M_LD_TRAY_SHIFTER[%02ld]"), l_Axis);
		break;
	case 4:   
		strMsg_1.Format(_T("M_UNLD_TRAY_SHIFTER[%02ld]"), l_Axis);
		break;
	case 5:   
		strMsg_1.Format(_T("M_EMPTY_TRAY_ELV[%02ld]"), l_Axis);
		break;
	case 6:   
		strMsg_1.Format(_T("M_LD_BUFF_ROTATOR[%02ld]"), l_Axis);
		break;
	case 7:   
		strMsg_1.Format(_T("M_LD_BUFF_Y[%02ld]"), l_Axis);
		break;
	case 8:   

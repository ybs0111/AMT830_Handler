// Run_Rear_HeatSnk_Stacker.cpp : 구현 파일입니다.
//
...
CRun_Rear_HeatSnk_Stacker clsRunHsRearStacker;
CRun_Rear_HeatSnk_Stacker::CRun_Rear_HeatSnk_Stacker()
{
	m_nInitStep = -1;
	m_nRunStep = -1;
	m_nAxisNum_Ready = M_HS_R_STACKER_READY;
...
void CRun_Rear_HeatSnk_Stacker::ThreadRun()
{
...
//kwlee 2017.0611
void CRun_Rear_HeatSnk_Stacker::OnReadyStackerMove()
{
	int nRet = 0;
	int nRet_1 = 0;
	switch(m_nRunStep)
	...
	case 200:
		...
		//kwlee 2017.0605
		if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_tray_chk,IO_OFF) == IO_OFF) 
		{
			m_nRunStep = 300;
		}
		else
		{
			m_strCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_hs_Rear_Ready_stacker_tray_chk);
	...
	case 1000:
		...
		{
			if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_updn_pos_chk,IO_ON) == IO_ON)
			{
				m_nRunStep = 1100;
			}
			else
	...
	case 2400:
		...
		//kwlee 2017.0605
		nRet = FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_tray_chk,IO_ON);
	...
	case 2600:
		...
		//kwlee 2017.0605
		if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_tray_chk,IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_tray_chk,IO_OFF) == IO_OFF)
		{
			OnSet_PusherFwdBwdCyl(IO_OFF);
...
void CRun_Rear_HeatSnk_Stacker::OnWorkStackerMove()
{
	int nRet = 0;
	...
	case 100:
		if (st_lot_info[LOT_CURR].nLotStatus == LOT_START)
		{
			...
			//kwlee 2017.0605
			nRet = FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_updn_pos_chk,IO_OFF);
			if (nRet == IO_OFF)
			{
			...
			{
				m_strCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_hs_Rear_Work_stacker_updn_pos_chk); 
	...
	case 700:
		if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_tray_chk,IO_OFF) == IO_OFF)
		{
		...
		{
			m_strCode.Format(_T("8%d%04d"), IO_ON, st_io_info.i_hs_Rear_Work_stacker_tray_chk); 
	...
	case 900:
		//kwlee 2017.0605
		if (st_sync_info.nRearHsRbt == CTL_REQ)
		{
	...
	case 1000:
		if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_tray_chk,IO_ON) == IO_ON && m_nWork_StackerPos == CTL_CHANGE)
		{
			//st_sync_info.nWorkRobot[THD_HS_FRONT_STACKER_SITE][STACKER_WORK_POS] = CTL_READY;
			//kwlee 2017.0605
			st_sync_info.nRearHsRbt = CTL_READY;
	...
	case 1400:
		if (st_Picker_info.nLastPick[THD_HS_REAR_RBT] == YES)
		{
			//m_nRunWorkstackerStep = 1300;
...
void CRun_Rear_HeatSnk_Stacker::OnRunInit()
{
	int nRet_1, nRet_2;

	if (st_sync_info.nInit_Flag[THD_HS_REAR_STACKER_SITE] != INIT_CLEAR)		return;	
	...
	case 100: //up
		if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_tray_chk,IO_OFF) == IO_ON)
		{	
			CTL_Lib.Alarm_Error_Occurrence(10,CTL_dWARNING, _T("810208"));
		}
		else if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_tray_chk,IO_OFF) == IO_ON)
		{
	...
	case 1100:
		st_sync_info.nInit_Flag[THD_HS_REAR_STACKER_SITE] = INIT_READY; 
		m_nInitStep = 0;
		break;	
...
void CRun_Rear_HeatSnk_Stacker::OnSet_ReadyPos_StackerCylUpDn(int nUpDn)
{
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Ready_stacker_updn,nUpDn);
	m_dwCylWaitTime[READY_STACKER_UPDN][0] = GetCurrentTime();
	m_bCylinderFlag[READY_STACKER_UPDN] = false;
}

int CRun_Rear_HeatSnk_Stacker::OnGet_ReadyPos_StackerCylUpDn(int nUpDn)
{
	...
	if (nUpDn == IO_ON)
	{
		if (m_bCylinderFlag[READY_STACKER_UPDN] == false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_dn_chk, IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_up_chk, IO_OFF) == IO_OFF)
		{
	...
	{
		if (m_bCylinderFlag[READY_STACKER_UPDN] == false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_dn_chk, IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_up_chk, IO_ON) == IO_ON)
		{
			m_bCylinderFlag[READY_STACKER_UPDN] = true;
			m_dwCylWaitTime[READY_STACKER_UPDN][0] = GetCurrentTime();
		}
		...
		else
		{
			...
			if (m_dwCylWaitTime[READY_STACKER_UPDN][2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				m_strCode.Format(_T("8%d%04d"), nUpDn, st_io_info.i_hs_Rear_Ready_stacker_up_chk); 
				return RET_ERROR;
			}
		}
	}
...
void CRun_Rear_HeatSnk_Stacker::OnSet_ReadyPos_RailFwdBwdCyl(int nOnOff)
{

	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Ready_stacker_Left_rail_fwd_onoff,nOnOff);
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Ready_stacker_Right_rail_fwd_onoff,nOnOff);

	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Ready_stacker_Left_rail_bwd_onoff,!nOnOff);
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Ready_stacker_Right_rail_bwd_onoff,!nOnOff);

	m_dwCylWaitTime[READY_RAIL_FWDBWD][0] = GetCurrentTime();
	m_bCylinderFlag[READY_RAIL_FWDBWD] = false;
}

int CRun_Rear_HeatSnk_Stacker::OnGet_ReadyPos_RailFwdBwdCyl(int nOnOff)
{
	...
	if (nOnOff == IO_ON)
	{
		if (m_bCylinderFlag[READY_RAIL_FWDBWD] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Left_rail_fwd_chk,IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Left_rail_bwd_chk,IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Right_rail_fwd_chk,IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Right_rail_bwd_chk,IO_OFF) == IO_OFF)
		{
		...
		else
		{
			...
			{
				if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Left_rail_fwd_chk,IO_ON) == IO_OFF)
				{
					m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_Ready_stacker_Left_rail_fwd_chk); 
				}
				else if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Right_rail_fwd_chk,IO_ON) == IO_OFF)
				{
					m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_Ready_stacker_Right_rail_fwd_chk); 
				}
				else
				{
	...
	{
		if (m_bCylinderFlag[READY_RAIL_FWDBWD] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Left_rail_fwd_chk,IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Left_rail_bwd_chk,IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Right_rail_fwd_chk,IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Right_rail_bwd_chk,IO_ON) == IO_ON)
		{
			m_bCylinderFlag[READY_RAIL_FWDBWD] = true;
			m_dwCylWaitTime[READY_RAIL_FWDBWD][0] = GetCurrentTime();
		}
		...
		else
		{
			...
			if (m_dwCylWaitTime[READY_RAIL_FWDBWD][2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Left_rail_bwd_chk,IO_ON) == IO_OFF)
				{
					m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_Ready_stacker_Left_rail_bwd_chk); 
				}
				else if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_Right_rail_bwd_chk,IO_ON) == IO_OFF)
				{
					m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_Ready_stacker_Right_rail_bwd_chk); 
				}
				else
				{
...
void CRun_Rear_HeatSnk_Stacker::OnSet_ReadyPos_TrayClampCyl(int nOnOff)
{
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Ready_stacker_guide_clamp_onoff,nOnOff);
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Ready_stacker_guide_unclamp_onoff,!nOnOff);

	m_dwCylWaitTime[READY_TRAYCLAMP_ONOFF][0] = GetCurrentTime();
	m_bCylinderFlag[READY_TRAYCLAMP_ONOFF] = false;
}

int CRun_Rear_HeatSnk_Stacker::OnGet_ReadyPos_TrayClampCyl(int nOnOff)
{
	int nWaitTime = WAIT_STACKER_CYL_ON_OFF;
	int nRet = RET_PROCEED;
	if (nOnOff == IO_ON)
	{
		if (m_bCylinderFlag[READY_TRAYCLAMP_ONOFF] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_guide_clamp_on_chk,IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_guide_unclamp_off_chk,IO_OFF) == IO_OFF)
	...
	{
		if (m_bCylinderFlag[READY_TRAYCLAMP_ONOFF] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_guide_clamp_on_chk,IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Ready_stacker_guide_unclamp_off_chk,IO_ON) == IO_ON)
		...
		else
		{
			...
			if (m_dwCylWaitTime[READY_TRAYCLAMP_ONOFF][2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_Ready_stacker_guide_unclamp_off_chk); 
				return RET_ERROR;
			}
		}
	}
...
void CRun_Rear_HeatSnk_Stacker::OnSet_PusherFwdBwdCyl(int nOnOff)
{
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_stacker_tray_pusher_fwd_onoff,nOnOff);
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_stacker_tray_pusher_bwd_onoff,!nOnOff);

	m_dwCylWaitTime[READY_PUSHER_ONOFF][0] = GetCurrentTime();
	m_bCylinderFlag[READY_PUSHER_ONOFF] = false;
}

int CRun_Rear_HeatSnk_Stacker::OnGet_PusherFwdBwdCyl(int nOnOff)
{
	int nWaitTime = WAIT_STACKER_CYL_ON_OFF;
	int nRet = RET_PROCEED;
	if (nOnOff == IO_ON)
	{
		if (m_bCylinderFlag[READY_PUSHER_ONOFF] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_stacker_tray_pusher_fwd_chk,IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_stacker_tray_pusher_bwd_chk,IO_OFF) == IO_OFF)
		...
		else
		{
			...
			{
				m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_stacker_tray_pusher_fwd_chk);
				return RET_ERROR;
			}
		}
	}
	...
	{
		if (m_bCylinderFlag[READY_PUSHER_ONOFF] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_stacker_tray_pusher_fwd_chk,IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_stacker_tray_pusher_bwd_chk,IO_ON) == IO_ON)
		...
		else
		{
			...
			if (m_dwCylWaitTime[READY_PUSHER_ONOFF][2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_stacker_tray_pusher_bwd_chk);
				return RET_ERROR;
			}
		}
	}
...
void CRun_Rear_HeatSnk_Stacker::OnSet_WorkPos_StackerCylUpDn(int nUpDn)
{
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Work_stacker_updn,nUpDn);
	m_dwCylWaitTime[WORK_STACKER_UPDN][0] = GetCurrentTime();
	m_bCylinderFlag[WORK_STACKER_UPDN] = false;
}

int CRun_Rear_HeatSnk_Stacker::OnGet_WorkPos_StackerCylUpDn(int nUpDn)
{
	int nWaitTime = WAIT_STACKER_UP_DN;
	int nRet = RET_PROCEED;
	if (nUpDn == IO_ON)
	{
		if (m_bCylinderFlag[WORK_STACKER_UPDN] == false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_dn_chk, IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_up_chk, IO_OFF) == IO_OFF)
		{
		...
		else
		{
			...
			{
				m_strCode.Format(_T("8%d%04d"), nUpDn, st_io_info.i_hs_Rear_Work_stacker_dn_chk);
				return RET_ERROR;
			}
		}
	}
	...
	{
		if (m_bCylinderFlag[WORK_STACKER_UPDN] == false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_dn_chk, IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_up_chk, IO_ON) == IO_ON)
		{
			m_bCylinderFlag[WORK_STACKER_UPDN] = true;
			m_dwCylWaitTime[WORK_STACKER_UPDN][0] = GetCurrentTime();
		}
		...
		else
		{
			...
			if (m_dwCylWaitTime[WORK_STACKER_UPDN][2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				m_strCode.Format(_T("8%d%04d"), nUpDn, st_io_info.i_hs_Rear_Work_stacker_up_chk);
				return RET_ERROR;
			}
		}
	}
...
void CRun_Rear_HeatSnk_Stacker::OnSet_WorkPos_RailFwdBwdCyl(int nOnOff)
{
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Work_stacker_Left_rail_fwd_onoff,nOnOff);
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Work_stacker_Right_rail_fwd_onoff,nOnOff);

	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Work_stacker_Left_rail_bwd_onoff,!nOnOff);
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Work_stacker_Right_rail_bwd_onoff,!nOnOff);

	m_dwCylWaitTime[WORK_RAIL_FWDBWD][0] = GetCurrentTime();
	m_bCylinderFlag[WORK_RAIL_FWDBWD] = false;
}

int CRun_Rear_HeatSnk_Stacker::OnGet_WorkPos_RailFwdBwdCyl(int nOnOff)
{
	int nWaitTime = WAIT_STACKER_CYL_ON_OFF;
	int nRet = RET_PROCEED;
	if (nOnOff == IO_ON)
	{
		if (m_bCylinderFlag[WORK_RAIL_FWDBWD] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Left_rail_fwd_chk,IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Left_rail_bwd_chk,IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Right_rail_fwd_chk,IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Right_rail_bwd_chk,IO_OFF) == IO_OFF)
		{
		...
		else
		{
			...
			{
				if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Left_rail_fwd_chk,IO_ON) == IO_OFF)
				{
					m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_Work_stacker_Left_rail_fwd_chk); 
				}
				else if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Right_rail_fwd_chk,IO_ON) == IO_OFF)
				{
					m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_Work_stacker_Right_rail_fwd_chk); 
				}
				else
				{
	...
	{
		if (m_bCylinderFlag[WORK_RAIL_FWDBWD] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Left_rail_fwd_chk,IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Left_rail_bwd_chk,IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Right_rail_fwd_chk,IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Right_rail_bwd_chk,IO_ON) == IO_ON)
		{
			m_bCylinderFlag[WORK_RAIL_FWDBWD] = true;
			m_dwCylWaitTime[WORK_RAIL_FWDBWD][0] = GetCurrentTime();
		}
		...
		else
		{
			...
			if (m_dwCylWaitTime[WORK_RAIL_FWDBWD][2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Left_rail_bwd_chk,IO_ON) == IO_OFF)
				{
					m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_Work_stacker_Left_rail_bwd_chk); 
				}
				else if (FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_Right_rail_bwd_chk,IO_ON) == IO_OFF)
				{
					m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_Work_stacker_Right_rail_bwd_chk); 
				}
				else
				{
...
void CRun_Rear_HeatSnk_Stacker::OnSet_WorkPos_TrayClampCyl(int nOnOff)
{
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Work_stacker_guide_clamp_onoff,nOnOff);
	FAS_IO.set_out_bit(st_io_info.o_hs_Rear_Work_stacker_guide_unclamp_onoff,!nOnOff);

	m_dwCylWaitTime[WORK_TRAY_CLAMP_ONOFF][0] = GetCurrentTime();
	m_bCylinderFlag[WORK_TRAY_CLAMP_ONOFF] = false;
}

int CRun_Rear_HeatSnk_Stacker::OnGet_WorkPos_TrayClampCyl(int nOnOff)
{
	int nWaitTime = WAIT_STACKER_CYL_ON_OFF;
	int nRet = RET_PROCEED;
	if (nOnOff == IO_ON)
	{
		if (m_bCylinderFlag[WORK_TRAY_CLAMP_ONOFF] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_guide_clamp_on_chk,IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_guide_unclamp_off_chk,IO_OFF) == IO_OFF)
		{
		...
		else
		{
			...
			{
				m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_Work_stacker_guide_clamp_on_chk); 
				return RET_ERROR;
			}
		}
	}
	...
	{
		if (m_bCylinderFlag[WORK_TRAY_CLAMP_ONOFF] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_guide_clamp_on_chk,IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_Work_stacker_guide_unclamp_off_chk,IO_ON) == IO_ON)
		...
		else
		{
			...
			if (m_dwCylWaitTime[WORK_TRAY_CLAMP_ONOFF][2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_Work_stacker_guide_unclamp_off_chk); 
				return RET_ERROR;
			}
		}
	}
...
int CRun_Rear_HeatSnk_Stacker::OnGet_SeperateFwdBwdCyl(int nOnOff)
{
	int nWaitTime = WAIT_STACKER_CYL_ON_OFF;
	int nRet = RET_PROCEED;
	if (nOnOff == IO_ON)
	{
		if (m_bCylinderFlag[WORK_TRAY_SEPERATE_ONOFF] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_stacker_tray_Seperate_fwd_chk,IO_ON) == IO_ON &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_stacker_tray_Seperate_bwd_chk,IO_OFF) == IO_OFF)
		{
		...
		else
		{
			...
			{
				m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_stacker_tray_Seperate_fwd_chk); 
				return RET_ERROR;
			}
		}
	}
	...
	{
		if (m_bCylinderFlag[WORK_TRAY_SEPERATE_ONOFF] ==  false && 
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_stacker_tray_Seperate_fwd_chk,IO_OFF) == IO_OFF &&
			FAS_IO.get_in_bit(st_io_info.i_hs_Rear_stacker_tray_Seperate_bwd_chk,IO_ON) == IO_ON)
		...
		else
		{
			...
			if (m_dwCylWaitTime[WORK_TRAY_SEPERATE_ONOFF][2] > (DWORD)st_wait_info.nLimitWaitTime[nWaitTime])
			{
				m_strCode.Format(_T("8%d%04d"), nOnOff, st_io_info.i_hs_Rear_stacker_tray_Seperate_bwd_chk); 
				return RET_ERROR;
			}
		}
	}
